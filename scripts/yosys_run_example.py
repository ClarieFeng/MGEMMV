...
def run_yosys(module_dir):
    ...
    """ Run Yosys to generate JSON file (Linux Version) """
    # 直接使用 yosys 命令，前提是当前 shell 环境已包含 yosys
    command = ['yosys', '-s', module_dir]

    try:
        # 使用 list 形式传递命令更安全，避免 shell 注入问题
        ...
        print("Error: 'yosys' command not found. Please ensure yosys is in your PATH or activate the correct environment.")
        return "Yosys not found"
